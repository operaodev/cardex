...
func NewSyncService(providersSvc *providers.Service, productsRepo products.Repository) *SyncService {
	return &SyncService{
...
func (s *SyncService) SyncAll(tcg products.TCG) (int, error) {
	log.Printf("[sync] Iniciando sincronización completa para TCG=%s", tcg)

	results, err := s.providersSvc.FetchItems(tcg)
	if err != nil {
	...
	upserted, err := s.productsRepo.Upsert(results)
	if err != nil {
...
func (s *SyncService) SyncByName(tcg products.TCG, name string) (int, error) {
	log.Printf("[sync] Iniciando sincronización por nombre para TCG=%s, Name=%s", tcg, name)

	results, err := s.providersSvc.FetchItemsByName(tcg, name)
	if err != nil {
