...
// Item representa una carta impresa (print) o un set dentro de un TCG.
//
// Identidad única de un print:  ExternalID + SetExternalID + TCG + Code + Lang + Rarity
// Identidad única de un set:    ExternalID + SetExternalID + TCG + Code + Lang + Rarity
// (mismo constraint unificado, campos vacíos actúan como discriminador)
type Item struct {
	ID   uint64   `json:"id"    gorm:"primaryKey;autoIncrement"`
	Type ItemType `json:"type"  gorm:"not null"`

	ExternalID    string   `json:"external_id"    gorm:"uniqueIndex:idx_item_identity,priority:1"`
	SetExternalID string   `json:"set_external_id" gorm:"uniqueIndex:idx_item_identity,priority:2"`
	TCG           TCG      `json:"tcg"             gorm:"size:20;not null;uniqueIndex:idx_item_identity,priority:3"`
	Code          string   `json:"code,omitempty"  gorm:"uniqueIndex:idx_item_identity,priority:4"`
	Lang          LangCode `json:"lang"            gorm:"size:2;not null;uniqueIndex:idx_item_identity,priority:5"`
	Rarity        string   `json:"rarity,omitempty" gorm:"size:60;uniqueIndex:idx_item_identity,priority:6"`
	...
	Wanted     uint   `json:"wanted"              gorm:"defa	ult:0;not null;index"` // feed principal
	...
	Edition        string `json:"edition,omitempty" gorm:"uniqueIndex:idx_item_identity,priority:7"`
...
type ItemUniqueKey string

// UniqueKey devuelve la clave compuesta que identifica de forma única un Item,
// espejando el índice idx_item_identity de la base de datos.
// Ejemplo: Ojama Yellow|Chazz Princeton|YGO|CHPR-EN01|EN|Common
func (i Item) UniqueKey() ItemUniqueKey {
	return ItemUniqueKey(strings.Join([]string{
		i.ExternalID,
		i.SetExternalID,
		string(i.TCG),
		i.Code,
